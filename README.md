...
promtail/                   log collection: docker\_sd, journal, /var/log
...
sudo ADMIN\_USER=catalin bash host/01-users.sh    # then verify: ssh catalin@<vps> from a NEW terminal
sudo ADMIN\_USER=catalin bash host/02-ssh-hardening.sh
sudo bash host/03-firewall.sh
sudo ADMIN\_USER=catalin bash host/04-docker.sh
sudo bash host/05-logging.sh
sudo ADMIN\_USER=catalin bash host/06-install-stack.sh
```

Access Grafana through an SSH tunnel (nothing but SSH is open to the internet):

```bash
ssh -L 3000:127.0.0.1:3000 -L 9090:127.0.0.1:9090 catalin@<vps>
...
* **Containers** (cAdvisor): CPU, working-set memory vs. `mem\_limit`, network, restarts.
...
|-|-|-|
|Container stdout/stderr|`docker\_sd\_configs` via Docker socket|`container`, `service`, `stream`|
...
|HostDiskWillFillIn24h|Prometheus|`predict\_linear` over 6h < 0|warning|
...
* **SSH hardening** (`/etc/ssh/sshd\_config.d/10-hardening.conf`): no root login, no passwords,
