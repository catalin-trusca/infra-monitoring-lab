...
sudo ADMIN_USER=catalin bash host/01-users.sh    # then verify: ssh catalin@<vps> from a NEW terminal
sudo ADMIN_USER=catalin bash host/02-ssh-hardening.sh
sudo bash host/03-firewall.sh
sudo ADMIN_USER=catalin bash host/04-docker.sh
sudo bash host/05-logging.sh
sudo ADMIN_USER=catalin bash host/06-install-stack.sh
```
